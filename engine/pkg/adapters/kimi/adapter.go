// Package kimi 实现月之暗面 Kimi API 适配器。
// Kimi 兼容 OpenAI 格式，鉴权使用 Bearer token。
package kimi
...
var pricingTable = map[string][2]float64{
	...
	"moonshot-v1-128k": {0.69, 0.69},
}

// Adapter 月之暗面 Kimi 适配器（OpenAI 兼容格式）
type Adapter struct {
...
func (a *Adapter) Vendor() vendor.Vendor       { return vendor.VendorKimi }
...
func (a *Adapter) ParseUsage(body []byte) (*vendor.Usage, error) {
	var r kimiResponse
	if err := json.Unmarshal(body, &r); err != nil {
		return nil, fmt.Errorf("parse kimi usage: %w", err)
	}
	return &vendor.Usage{
		PromptTokens: r.Usage.PromptTokens, CompletionTokens: r.Usage.CompletionTokens,
		TotalTokens: r.Usage.TotalTokens,
